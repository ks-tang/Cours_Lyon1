...
public class TP2MainChang100 {
    public static void main(String[] args){ // On déclare le programme principal
        ...
        tp.setDefaultNodeModel(TP2NoeudChang100.class);  
        tp.setOrientation(Link.Orientation.DIRECTED);

        Point centre = new Point(tp.getWidth()/2, tp.getHeight()/2);
        double rayon = 40;
        for (Integer i = 0; i < TP2NoeudChang100.TAILLE_ANNEAU; i++) {
            Node noeud = new TP2NoeudChang100();
            double alpha = (2 * Math.PI / TP2NoeudChang100.TAILLE_ANNEAU) * i;
        ...
        for (Integer i = 0; i < tp.getNodes().size(); i++) {
            ...
            tp.addLink(new Link(
                tp.getNodes().get(i), 
                tp.getNodes().get((i+1) % TP2NoeudChang100.TAILLE_ANNEAU), 
