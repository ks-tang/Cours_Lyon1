...
public class TP3MainCooTourn {
    public static void main(String[] args){ // On déclare le programme principal
        ...
        tp.setDefaultNodeModel(TP3NoeudCooTourn.class);  

        Point centre = new Point(tp.getWidth()/2, tp.getHeight()/2);
        double rayon = 40;
        for (Integer i = 0; i < TP3NoeudCooTourn.TAILLE_ANNEAU; i++) {
            Node noeud = new TP3NoeudCooTourn();
            double alpha = (2 * Math.PI / TP3NoeudCooTourn.TAILLE_ANNEAU) * i;
