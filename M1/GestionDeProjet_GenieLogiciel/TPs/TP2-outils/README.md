...
(_issue_) intitulée : “ignorer le répertoire target et les fichiers des
...
typique pour un petit projet (_pas_ pour le nôtre) ressemble à ceci :
...
warnings et pas seulement pour les erreurs. On demande à exécuter pendant `mvn test` (via `<phase>test</phase>`) la cible `check` (`<goal>check</goal>`) du
